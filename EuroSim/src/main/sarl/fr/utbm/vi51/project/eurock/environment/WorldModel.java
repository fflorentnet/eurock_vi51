...
public class WorldModel extends AbstractEnvironment implements WorldModelStateProvider {
	...
	public void createArtist() {
		...
		addAgentBody(
				body,
				randomPositionArtist(),
				(float) Math.random() * MathUtil.TWO_PI);
	}
	
	protected Point2f randomPositionArtist() {
		float x = (float) Math.random() * getWidth() - ARTIST_SIZE;
		float y = (float) Math.random() * getHeight() - ARTIST_SIZE;
	...
	protected Point2f randomPositionSpectator() {
		float x = (float) Math.random() * getWidth() - ARTIST_SIZE;
		float y = (float) Math.random() * getHeight() - ARTIST_SIZE;
		return new Point2f(x, y);
	}
	...
	public void createSecurityAgent() {
		...
		addAgentBody(
				body,
				randomPositionSecurityAgent(),
				(float) Math.random() * MathUtil.TWO_PI);
	}
	
	protected Point2f randomPositionSecurityAgent() {
		float x = (float) Math.random() * getWidth() - ARTIST_SIZE;
		float y = (float) Math.random() * getHeight() - ARTIST_SIZE;
		return new Point2f(x, y);
	}
